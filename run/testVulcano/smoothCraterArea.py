...
def read_asc_with_pandas(filepath):
    """Read a raster .asc file and return the data and metadata."""
...
def replace_area_with_interpolation(data, x, y, radius, header):
    ...
    xy = np.concatenate([border_x.reshape(-1, 1),
                         border_y.reshape(-1, 1)],
                        axis=1)
...
def plot_modified_points_3d(data,
                            mask,
                            x_coords,
                            y_coords,
                            border_x,
...
def main():
    ...
    try:
        ...
        print(
            f"r_crater_top: {r_crater_top}, Xvent: {x_vent}, Yvent: {y_vent}")
    ...
    new_data, mask, border_x, border_y, border_values = (
        replace_area_with_interpolation(data, point_x, point_y, radius,
                                        header))
